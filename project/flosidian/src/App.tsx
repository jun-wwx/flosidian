...
// localStorage keys
const NOTES_KEY = 'neuronote_notes';
const CONTENT_PREFIX = 'neuronote_content_';
...
const getNotes = (): NoteInfo[] => {
  const data = localStorage.getItem(NOTES_KEY);
  return data ? JSON.parse(data) : [];
};

const saveNotesList = (notes: NoteInfo[]) => {
  localStorage.setItem(NOTES_KEY, JSON.stringify(notes));
};

const getNoteContent = (name: string): string => {
  return localStorage.getItem(CONTENT_PREFIX + name) || '';
};

const saveNoteContent = (name: string, content: string) => {
  localStorage.setItem(CONTENT_PREFIX + name, content);
};

const deleteNoteContent = (name: string) => {
  localStorage.removeItem(CONTENT_PREFIX + name);
};
...
function App() {
  ...
  const loadNotes = useCallback(() => {
    ...
    setNotes(notesList);
    extractTags(notesList);
    buildGraph(notesList);
  }, []);

  // Extract all tags from notes
  const extractTags = (notesList: NoteInfo[]) => {
    const tagMap = new Map<string, number>();
    for (const note of notesList) {
      const noteContent = getNoteContent(note.name);
  ...
  // Build graph data
  const buildGraph = (notesList: NoteInfo[]) => {
    const nodes: GraphNode[] = notesList.map(n => ({ id: n.name, name: n.name, val: 1 }));
    const links: GraphLink[] = [];
    
    for (const note of notesList) {
      const noteContent = getNoteContent(note.name);
      const linkMatches = noteContent.match(/\[\[([^\]]+)\]\]/g) || [];
  ...
  // Load a note
  const loadNote = useCallback((name: string) => {
    const noteContent = getNoteContent(name);
    ...
    for (const note of notesList) {
      if (note.name !== name) {
        const noteContent2 = getNoteContent(note.name);
        if (noteContent2.includes(`[[${name}]]`)) {
    ...
    if (editorViewRef.current) {
      editorViewRef.current.dispatch({
        changes: { from: 0, to: editorViewRef.current.state.doc.length, insert: noteContent }
      });
    }
  ...
  const saveNote = useCallback((newContent: string) => {
    ...
    saveNoteContent(currentNote, newContent);
    loadNotes();
  ...
  const createNote = () => {
    ...
    const notesList = getNotes();
    if (notesList.some(n => n.name === newNoteName.trim())) {
      alert('Note already exists!');
      return;
    }
    
    const newNote: NoteInfo = { name: newNoteName.trim(), path: newNoteName.trim() };
    notesList.push(newNote);
    saveNotesList(notesList);
    saveNoteContent(newNote.name, '# ' + newNote.name + '\n\n');
    
    setShowNewNoteDialog(false);
    setNewNoteName('');
    loadNotes();
    loadNote(newNote.name);
  };
  ...
  const deleteNote = (name: string) => {
    ...
    const notesList = getNotes();
    const newNotes = notesList.filter(n => n.name !== name);
    saveNotesList(newNotes);
    deleteNoteContent(name);
  ...
  const searchNotes = useCallback((query: string) => {
    ...
    for (const note of notesList) {
      ...
      if (note.name.toLowerCase().includes(lowerQuery) || 
          noteContent.toLowerCase().includes(lowerQuery)) {
  ...
  const handleTagClick = (tagName: string) => {
    ...
    for (const note of notesList) {
      const noteContent = getNoteContent(note.name);
      if (noteContent.includes(`#${tagName}`)) {
